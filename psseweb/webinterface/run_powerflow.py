...
def powerFlow_result_process(result: dict,
    ...
    logfilepath = f"{user_folder}/PowerFlow/{savfile}/{savfile}_log.txt"
        ...
        #==========================================================================    
            ...
            if mismatch!=[] and iterations!=[]:
                ...
                if float(mismatch_value[0])< convergence_threshold:
                    ...
                    if iferror["error"]:
                        return_value.append({"content":f"PowerFlow-{savfile}.sav執行成功，但寫入log失敗: mismatch: {mismatch_value[0]} MVA, loop_times:{str(loop_times+1)}",
                                        "errorcode": 0 }) 
                        ...
                        print('end')
                        return_value.append({"content":f"PowerFlow-{savfile}: mismatch: {mismatch_value[0]} MVA, loop_times:{str(loop_times+1)}",
                                            "errorcode": 0 }) 
...
def RUN_PowerFlow_for_many_saveFile(    UserFolder
    ...
    savfilelocation = UserFolder+'SavFile/'
    ...
    for savfile in Sav_File_list:
        ...
        for loop_times in range(5):
            ...
            print(savfile,'-->',loop_times,'-->',processed_result)                                    
...
def Batch_RUN_Psse_for_rawFile( savfile
                                ...
                                ,maxbasekv
                                ,confile_type):
    print(savfile, ' ' ,UserFolder, ' ', user, ' ' ,convergence)
