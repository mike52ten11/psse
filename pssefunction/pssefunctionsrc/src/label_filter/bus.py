...
@error_handler
def bus(raw_data, rawfilepath, npzfilepath,filter_dir,zone_data_dict):
    ...
    np.savez(f'{npzfilepath}', name=bus_name, num=bus_number,zonenum=zone_number,zonename=zone_name) 
    ...
    with open(f'{filter_dir}/bus_data.txt', 'w', encoding='utf-8') as f:
        for bnum, bname,znum,zname in zip(bus_number,bus_name,zone_number,zone_name):
            f.write(f"{bnum},{bname},{znum},{zname}\n")
