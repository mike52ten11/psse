...
class PowerFlowSubThread(threading.Thread):
    def __init__(self, busnumber: str, busname: str, savfile_name: str, 
                 user: str, powerflow_folder: str, source_Folder: str, 
                 target_Folder: str, contxt: str, return_value: List[Dict],log_dir:str,systemlog):
        super().__init__()
    ...
    def run(self):
        try:
            # 解析 busnumber（可能是單一數字或組合）
            busnumber_str = self.busnumber  # 例如 "3070" 或 "3070,3071"
            self.systemlog.info(f' N2跑的排列組合 {busnumber_str}')
            # print('busnumber_str-->',busnumber_str)
            # busnumber_list = [int(x) for x in busnumber_str.split(',')]  # 轉為整數列表  
            # print('busnumber_list -->',busnumber_list)          
            # 構建命令參數
            args = (f" --Sav_File {self.savfile_name}"
                   ...
                   f" --busnumber {busnumber_str}")

            cmd = Run_pyfile_by_execmd(
                python_location='python',
                pyfile='webinterface/src/Powerflow_of_subline_345n2.py',
                args=args
            )
            # print('cmd --> ', cmd)
            ...
            with self.lock:
                if not result["error"]:
                    ...
                    if os.path.exists(f"{self.target_Folder}/{self.busnumber.replace(',', '+')}.acc"):

                        self.return_value.append({
                            "content": f"{self.contxt} ---關{self.busnumber}_{self.busname}完成:已產出{self.busnumber.replace(',', '+')}.acc",
                            "errorcode": 0
                        })
                    else:
                        self.return_value.append({
                            "content": f"{self.contxt} ---關{self.busnumber}_{self.busname}成功:psse 產生acc檔失敗，請下載log，未產出{self.busnumber.replace(',', '+')}.acc",
                            "errorcode": 0
                        })                           
                else:
                    self.return_value.append({"content": f"PowerFlow-{self.savfile_name}: 錯誤"})
                    print('result ==>', result)
                    self.systemlog.info(result["return_value"]['backend_message'])
                    self.return_value.append({
                        "content": result["return_value"]["front_message"],
                        "errorcode": 1
                    })
...
def run_powerflow_threads(busnum: List[int], busname: List[str], 
                         ...
                         source_Folder: str, target_Folder: str, contxt: str, log_dir: str, systemlog) -> List[Dict]:
    return_value = []
    threads = []

    # 將 busnum 和 busname 對應成字典，方便查找
    bus_dict = dict(zip(busnum, busname))
    ...
    # 2. 處理兩兩組合 (C(n,2))
    for busnum_pair in combinations(busnum, 2):  # 生成所有兩兩組合
        busnumber_1, busnumber_2 = busnum_pair
        busname_1 = bus_dict[busnumber_1]
        busname_2 = bus_dict[busnumber_2]
        combined_busnumber = f"{busnumber_1},{busnumber_2}"  # 例如 "3070,3071"
        combined_busname = f"{busname_1}+{busname_2}"  # 例如 "Bus3070+Bus3071"

        thread = PowerFlowSubThread(
            busnumber=combined_busnumber,  # 傳遞組合的 busnumber 字串
            busname=combined_busname,      # 傳遞組合的 busname
            savfile_name=savfile_name,
            user=user,
            powerflow_folder=powerflow_folder,
            source_Folder=source_Folder,
...
def find_subline_in_busname(cmdline_messages,filtervalue,busdata):
    ...
    for busnumindex, busnameindex in zip(filtervalue["busnum"],filtervalue['busname']):
        ...
        elif busnumindex == 3069:
            busnum.append(busnumindex)
            busname.append(busnameindex)

        elif busnumindex == 3070:
            busnum.append(busnumindex)
            busname.append(busnameindex)

        elif busnumindex == 3071:
            busnum.append(busnumindex)
            busname.append(busnameindex)
...
def Run_Powerflow_of_subline(source_Folder
    ...
    for savfile in Sav_File:
        ...
        area_and_zone = f"area={'+'.join(area_num)},zone={'+'.join(zone_num)}"
        print(area_and_zone)
        target_Folder = f"../Data/User/{user}/PowerFlowSub/{savfile_name}/345KV_N-2/close/{area_and_zone}"
        print(savfile_name)
        print('areanum -->' ,area_num,'\n')
        create_subline_npz  (   target = target_Folder, 
                                zonenum = zone_num, 
                                areanum = area_num, 
                                minbasekv = minbasekv, 
                                maxbasekv = maxbasekv, 
        ...
        # busdata = np.load(f"../Data/User/{user}/filter/PowerFlow/bus/bus_{savfile_name}.npz")
        ...
        if  len(busnum)>0:
            ...
            # return_value.append([{"content":f'PowerFlow-{savfile}  有分歧',"errorcode": 0}])   
        else:
            print('沒有分歧')    
        ...
        #     print(cmd)
        #     logger_powerflow_subline.info(f"FUNCTION: webinterface/src/Run_Powerflow_of_subline_345KV_N1，MESSAGE:找 {areanum} in {area_num}的所有bus ，PROCESS: {cmd}")
        #     result = fileprocess.execCmd(cmd)

        #     if result["error"]:
        #         return_value.append([{"content":f"PowerFlow-{savfile}: 錯誤"}])
        #         iferror=1                
        #         logger_powerflow_subline.error(f"FUNCTION: webinterface/src/Run_Powerflow_of_subline_345KV_N1，MESSAGE:ERROR Ocurred: {result['return_value']} ，PROCESS: {cmd}")
             
  
        #     else:
        #         logger_powerflow_subline.info(f"FUNCTION: webinterface/src/Run_Powerflow_of_subline_345KV_N1，MESSAGE:執行成功結果 ，PROCESS: {cmd}")
        #         result_test = result["return_value"].decode("ansi",errors='ignore').split('\n')

        #         myArch = np.load(f"{target_Folder}/close/Area_{areanum}/分歧線的BusName與BusNum.npz")
        ...
        #                         log_dir = f'{target_Folder}/close/Area_{areanum}/log/',
        #                         systemlog = logger_powerflow_subline)
        #             return_value.append(results)              
        ...
        #     print(cmd)
        #     logger_powerflow_subline.info(f"FUNCTION: webinterface/src/run_powerflow_of_subline_345n1，MESSAGE:找 {zonenum} in {zone_num}的所有bus ，PROCESS: {cmd}")
        #     result = fileprocess.execCmd(cmd)

        #     if result["error"]:
        #         return_value.append([{"content":f"PowerFlow-{savfile}: 錯誤"}])
        #         iferror=1                
        #         logger_powerflow_subline.info(f"ERROR: FUNCTION: webinterface/src/Run_Powerflow_of_subline_345KV_N1，MESSAGE:ERROR Ocurred: {result['return_value']} ，PROCESS: {cmd}")
        ...
        #         myArch = np.load(f"{target_Folder}/close/Zone_{zonenum}/分歧線的BusName與BusNum.npz")
        ...
        #                         log_dir = f'{target_Folder}/close/Zone_{zonenum}/log/',
        #                         systemlog = logger_powerflow_subline)
        #             return_value.append(results)   
