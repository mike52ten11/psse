...
def How_many_npz_in_UserFolder(sourcedir:str) -> typing.Iterator: 
...
def How_many_out_in_dir(sourcedir:str):
    outfile = [] 
    for savfilename in os.listdir(sourcedir):
        files = fileprocess.How_many_rawfile(f"{sourcedir}/{savfilename}" , fileextension = r'.out')

        outfile.append(files[0])
    # files = fileprocess.How_many_rawfile(sav_folder, fileextension = r'.sav')

    # ic(outfile[0])
    return (_.split('.')[0] for _ in outfile
            )          
